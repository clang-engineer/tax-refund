...
- API Gateway패턴: 서비스로 전달되는 모든 API요청의 관문 역활을 하는 서버를 두는 방식. 시스템의 아키텍처를 내부로 숨기고 외부의 요청에 대한 응답만을 적절한 형태로 응답
- Service Discovery: 클라우드 환경에서 autoscaling 생성, 삭제, 확장 과정에서 동적으로 변경되는 인스턴스의 IP,PORT를 쉽게 찾기 위한 패턴s
